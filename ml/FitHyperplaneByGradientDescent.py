...
class FitHyperplaneByGradientDescent():
    def __init__(self):
        self.X = None
        self.y = None
        self.intercept = None
    ...
    class Dial(int):
        def __init__(self, *args):
            if len(args) == 1:
                ...
                raise ValueError("this function takes one argument only")
    ...
    def _mse(self, slope_angles, return_intercept_and_weights=False):
        """mean squared error"""
        assert hasattr(slope_angles, '__iter__'), "you must provide an array of slope angles"
        ...
        mse = ((yy_true - yy_pred) ** 2).sum() / len(self.y)
        return mse if not return_intercept_and_weights else (mse, intercept, weights)
    ...
    def fit(self, X,y):
        ...
        while True:
            for dim in range(1, self.X.shape[1]+1):
                table[1:] = table[0]
                dial = self.Dial(table[0,dim])
                table[1,dim] = abs(dial + 1)
                table[2, dim] = abs(dial - 1)
                table[1,0] = self._mse(table[1,1:])
                table[2,0] = self._mse(table[2, 1:])
                table = table[argsort(table[:,0])]
                l.append(table[0,0])
            if len(set(l[-n:]))==1: break
            k += 1
            if k > 180 * self.X.shape[1]:
                print("SAFETY BREAK")
                break
...
def main():
    ...
    md = FitHyperplaneByGradientDescent()
    md = md.fit(X,y)
